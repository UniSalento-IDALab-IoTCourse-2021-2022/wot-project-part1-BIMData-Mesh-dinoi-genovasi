            ...
            switch (scan_result->scan_rst.search_evt) {
                case ESP_GAP_SEARCH_INQ_RES_EVT:
                    if(esp_ble_is_ibeacon_packet(scan_result->scan_rst.ble_adv,scan_result->scan_rst.adv_data_len)){
                        ...
                        ESP_LOGI(GATTC_TAG,"----- TROVATO IL BEACON ----");
                        //esp_log_buffer_hex("UUID: ",beacon_data->ibeacon_vendor.proximity_uuid,ESP_UUID_LEN_128);
                        //ESP_LOGI(GATTC_TAG,"Measured Power: %d",beacon_data->ibeacon_vendor.measured_power);
                        ...
                        //ESP_LOGI(GATTC_TAG,"Major: %d Minor: %d",major,minor);
                        //ESP_LOGI(GATTC_TAG,"RSSI: %d",scan_result->scan_rst.rssi);
                        //ESP_LOGI(GATTC_TAG,"\n");

                        update_ibeacon_state(beacon_data->ibeacon_vendor.proximity_uuid, major, minor, scan_result->scan_rst.rssi);
                    }
